startTime <- Sys.time()
cat(paste0("> Rscript cmp_DE_TADs_genes_groupGOsemSimThreshGO.R\n"))

# Rscript cmp_DE_TADs_genes_groupGOsemSimThreshGO.R
...
outFold <- file.path("CMP_TADs_GENES_groupGOsemSimThreshGO", paste0(topPercent,"_", nTopSharedGO), groupGO_ont)
if(!SSHFS) system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "cmp_tads_genes_groupgo_semsimthreshgo_logFile.txt")
...
printAndLog(txt, logFile)
txt <- paste0("... nTopSharedGO:\t", nTopSharedGO, "\n")
printAndLog(txt, logFile)
...
if(buildTable){
  all_ds_DT <- foreach(curr_ds = all_ds, .combine='rbind') %do% {
    ...
    ######################################################## 
      ...
      # added here for quicker results
      topTADs_genes_groupGO_sharedGO <- topTADs_genes_groupGO_sharedGO[1:min(c(length(topTADs_genes_groupGO_sharedGO), nTopSharedGO))]
      ...
      # not true for the quicker result version
      ...
      # added here for quicker results
      topGenes_manyAsTopTADs_groupGO_sharedGO <- topGenes_manyAsTopTADs_groupGO_sharedGO[1:min(c(length(topGenes_manyAsTopTADs_groupGO_sharedGO), nTopSharedGO))]
      ...
      # added here for quicker results
      topGenes_manyAsTopPercent_groupGO_sharedGO <- topGenes_manyAsTopPercent_groupGO_sharedGO[1:min(c(length(topGenes_manyAsTopPercent_groupGO_sharedGO), nTopSharedGO))]
      ...
      result_list <- list(
        topTADs_genes_groupGO_resultDT=topTADs_genes_groupGO_resultDT,
        topGenes_manyAsTopTADs_groupGO_resultDT=topGenes_manyAsTopTADs_groupGO_resultDT,
        topGenes_manyAsTopPercent_groupGO_resultDT=topGenes_manyAsTopPercent_groupGO_resultDT
      )
      
      outFile <- file.path(outFold, paste0(curr_ds, "_groupGO_all_results_list.Rdata"))
      save(result_list, file = outFile)
  ...
  orderDataset <- all_ds_DT$dataset[order(all_ds_DT$topTADs_genes_groupGO_nSharedGO,
                                          all_ds_DT$topGenes_manyAsTopPercent_groupGO_nSharedGO,
                                          all_ds_DT$topGenes_manyAsTopTADs_groupGO_nSharedGO,
                                          decreasing=T)]
