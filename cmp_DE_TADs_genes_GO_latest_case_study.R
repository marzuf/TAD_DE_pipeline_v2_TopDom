startTime <- Sys.time()
cat(paste0("> Rscript cmp_DE_TADs_genes_GO_pvalSelect_latest.R\n"))

# Rscript cmp_DE_TADs_genes_GO_pvalSelect_latest.R
...
outFold <- "CMP_TADs_GENES_GO_latest_CASE_STUDY"
...
eccentricity(selectTADs_genes_g)

# for visualization, select the 20 top GOs
nTop <- 20

signifGOterm_pvals <- setNames(  selectTADs_genes_resultDT$p.adjust[order(selectTADs_genes_resultDT$p.adjust, decreasing=FALSE)],
                                 tolower(gsub("_", " ", gsub("^GO_", "", selectTADs_genes_resultDT$ID[order(selectTADs_genes_resultDT$p.adjust, decreasing=FALSE)]))))

# -> because I want to subset only those in minimal_set...
stopifnot(selectTADs_genes_signifGOmin %in% GOterm_GOid_cP)
selectTADs_genes_signifGOminTerm <- sapply(selectTADs_genes_signifGOmin, function(x)  names(GOterm_GOid_cP[GOterm_GOid_cP == x]))
signifGOminTerm_pvals <- signifGOterm_pvals[names(signifGOterm_pvals) %in% selectTADs_genes_signifGOminTerm]
length(signifGOminTerm_pvals)
length(selectTADs_genes_signifGOminTerm)
topSignifGOminTerm_pvals <- sort(signifGOminTerm_pvals, decreasing=FALSE)[1:nTop]
topSignifGOid <-  GOterm_GOid_cP[names(GOterm_GOid_cP) %in% topSignifGOminTerm_pvals]



# retrieve the p-values of the signifGOmin
signifGOminTerm <- names(selectTADs_genes_signifGOid[selectTADs_genes_signifGOid %in% selectTADs_genes_signifGOmin])

stopifnot(! any(duplicated(selectTADs_genes_resultDT$ID)))

stopifnot(signifGOminTerm %in% names(signifGOterm_pvals))
signifGOminTerm_pvals <- 

topSignifGOterm_pvals <- names(sort(signifGOterm_pvals, decreasing=FALSE))

  


selectTADs_genes_topSignifGOterm <-      
  all(selectTADs_genes_signifGOterm_tmp %in% names(GOterm_GOid_cP))
