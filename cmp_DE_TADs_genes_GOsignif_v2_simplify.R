cat(paste0("> Rscript cmp_DE_TADs_genes_GOsignif_v2_simplify.R\n"))

# Rscript cmp_DE_TADs_genes_GOsignif_v2_simplify.R
...
# settings for enrichment analysis: clusterProfiler::enrichGO
# default values enrichGO function: (universe as background !)
# pvalueCutoff = 0.05 
# pAdjustMethod = "BH"
# minGSSize = 10
# maxGSSize = 500
# qvalueCutoff = 0.2 
enrichGO_pvalueCutoff <- 1
enrichGO_pAdjustMethod <- "BH"
enrichGO_minGSSize <- 1
enrichGO_maxGSSize <- 500
enrichGO_qvalueCutoff <- 1
enrichGO_orgDB <- "org.Hs.eg.db"
enrichGO_keytype <- "ENTREZID"
enrichGO_ont <- "BP"
# whether mapping gene ID to gene Name
enrichGO_readable <- FALSE
# If ont='ALL', whether pool 3 GO sub-ontologies
enrichGO_pool <- FALSE

enrichGO_results_sortGOby <- "p.adjust"
enrichGO_results_cmpNbrTop <- 50

mySub <- paste0("(# top GO: ",enrichGO_results_cmpNbrTop, ")") 

outFold <- file.path("CMP_TADs_GENES_GO_SIGNIF_v2_WITH_SIMPLIFY", enrichGO_ont)
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "cmp_tads_genes_go_signif_v2_simplify_logFile.txt")
...
printAndLog(txt, logFile)
txt <- paste0("... enrichGO background genes:\t", "universe", "\n")
printAndLog(txt, logFile)
txt <- paste0("... enrichGO pvalueCutoff:\t", enrichGO_pvalueCutoff, "\n")
printAndLog(txt, logFile)
txt <- paste0("... enrichGO pAdjustMethod:\t", enrichGO_pAdjustMethod, "\n")
printAndLog(txt, logFile)
txt <- paste0("... enrichGO minGSSize:\t", enrichGO_minGSSize, "\n")
printAndLog(txt, logFile)
txt <- paste0("... enrichGO maxGSSize:\t", enrichGO_maxGSSize, "\n")
printAndLog(txt, logFile)
txt <- paste0("... enrichGO qvalueCutoff:\t", enrichGO_qvalueCutoff, "\n")
printAndLog(txt, logFile)
...
if(buildTable){
  # all_ds_DT <- foreach(curr_ds = all_ds, .combine='rbind') %dopar% {
    ...
    #***** 1) topTADs_genes
    cat("... start topTADs_genes_enrichGO\n")
    topTADs_genes_enrichGO <- enrichGO(gene=topTADs_genes, 
                                       OrgDb=enrichGO_orgDB, 
                                       keytype = enrichGO_keytype, 
                                       ont = enrichGO_ont,
                                       pvalueCutoff = enrichGO_pvalueCutoff, 
                                       pAdjustMethod = enrichGO_pAdjustMethod, 
                                       qvalueCutoff = enrichGO_qvalueCutoff,
                                       minGSSize = enrichGO_minGSSize, 
                                       maxGSSize = enrichGO_maxGSSize, 
                                       readable = enrichGO_readable, 
                                       pool = enrichGO_pool)
    
    cat("... start simplify topTADs_genes_enrichGO\n")
    topTADs_genes_enrichGO <- simplify(topTADs_genes_enrichGO)
    
    
    topTADs_genes_resultDT <- topTADs_genes_enrichGO@result
    topTADs_genes_resultDT <- topTADs_genes_resultDT[order(topTADs_genes_resultDT[,enrichGO_results_sortGOby], decreasing=FALSE), ]
    
    topTADs_genes_nTop <- min(c(enrichGO_results_cmpNbrTop, nrow(topTADs_genes_resultDT)))
    ...
    cat("... start topGenes_manyAsTopPercent_enrichGO\n")
    topGenes_manyAsTopPercent_enrichGO <- enrichGO(gene=topGenes_manyAsTopPercent, 
                                       OrgDb=enrichGO_orgDB, 
                                       keytype = enrichGO_keytype, 
                                       ont = enrichGO_ont,
                                       pvalueCutoff = enrichGO_pvalueCutoff, 
    ...
    cat("... start simplify topGenes_manyAsTopPercent_enrichGO\n")
    topGenes_manyAsTopPercent_enrichGO <- simplify(topGenes_manyAsTopPercent_enrichGO)
    
    
    
    
    topGenes_manyAsTopPercent_resultDT <- topGenes_manyAsTopPercent_enrichGO@result
    topGenes_manyAsTopPercent_resultDT <- topGenes_manyAsTopPercent_resultDT[order(topGenes_manyAsTopPercent_resultDT[,enrichGO_results_sortGOby], decreasing=FALSE), ]
    
    topGenes_manyAsTopPercent_nTop <- min(c(enrichGO_results_cmpNbrTop, nrow(topGenes_manyAsTopPercent_resultDT)))
    ...
    cat("... start topGenes_manyAsTopTADs_enrichGO\n")
    topGenes_manyAsTopTADs_enrichGO <- enrichGO(gene=topGenes_manyAsTopTADs, 
                                                   OrgDb=enrichGO_orgDB, 
                                                   keytype = enrichGO_keytype, 
                                                   ont = enrichGO_ont,
                                                   pvalueCutoff = enrichGO_pvalueCutoff, 
    ...
    cat("... start simplify topGenes_manyAsTopTADs_enrichGO\n")
    topGenes_manyAsTopTADs_enrichGO <- simplify(topGenes_manyAsTopTADs_enrichGO)
    
    topGenes_manyAsTopTADs_resultDT <- topGenes_manyAsTopTADs_enrichGO@result
    topGenes_manyAsTopTADs_resultDT <- topGenes_manyAsTopTADs_resultDT[order(topGenes_manyAsTopTADs_resultDT[,enrichGO_results_sortGOby], decreasing=FALSE), ]
    
    topGenes_manyAsTopTADs_nTop <- min(c(enrichGO_results_cmpNbrTop, nrow(topGenes_manyAsTopTADs_resultDT)))
    ...
    outFile <- file.path(outFold, paste0(curr_ds, "_enrichGO_all_results.Rdata"))
    enrichGO_all_results <- list(topTADs_genes = topTADs_genes_resultDT,
                                 topGenes_manyAsTopPercent = topGenes_manyAsTopPercent_resultDT,
                                 topGenes_manyAsTopTADs = topGenes_manyAsTopTADs_resultDT
                             )
    save(enrichGO_all_results, file = outFile)
