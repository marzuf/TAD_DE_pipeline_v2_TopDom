startTime <- Sys.time()
cat(paste0("> Rscript cmp_DE_TADs_genes_KEGG_pvalSelect.R\n"))

# Rscript cmp_DE_TADs_genes_KEGG_pvalSelect.R
...
outFold <- "CMP_TADs_GENES_KEGG_pvalSelect"
...
all_ds_DT$nKEGG_selectGenes <- (all_ds_DT$nbrKEGG_selectTADs_genes + all_ds_DT$nbrKEGG_selectGenes)

all_ds_DT$ratio_selectGenes <- all_ds_DT$intersectNbrKEGG_selectTADs_genes_selectGenes/all_ds_DT$nKEGG_selectGenes
...
all_cmp_var <- paste0(all_vars_to_plot, "_", c("selectTADs_genes","selectGenes"))

all_cmp_var <- c(all_cmp_var, 
                 "intersectNbrKEGG_selectTADs_genes_selectGenes",
                 "ratio_selectGenes"
)
...
for(curr_plot in toplot) {
    plotDT <- all_ds_DT[,c("dataset", "nKEGG_selectGenes")]
    plotTit <- paste0("# KEGG and intersect - DE genes")
  ...
  outFile <- file.path(outFold, paste0("KEGG_at_intersect_", curr_plot, "selectGenes.", plotType))
...
for(curr_plot in toplot) {
  ...
  p_var <- ggplot(plotDT_m, aes(x = dataset, y = value, fill = variable)) +
    ggtitle(plotTit, subtitle = paste0(mySub, " - orderFCC"))+
    geom_bar(stat="identity", position = "dodge")+
    scale_x_discrete(name="")+
    scale_y_continuous(name=paste0(""),
                       breaks = scales::pretty_breaks(n = 10))+
  ...
  outFile <- file.path(outFold, paste0("KEGG_at_intersect_", curr_plot, "selectGenes_orderFCC.", plotType))
  ggsave(plot = p_var, filename = outFile, height=myHeightGG, width = myWidthGG)
  cat(paste0("... written: ", outFile, "\n"))
