startTime <- Sys.time()
cat(paste0("> Rscript cmp_DE_TADs_genes_GOsignifEntropy_pvalSelect.R\n"))

# Rscript cmp_DE_TADs_genes_GOsignifEntropy_pvalSelect.R
...
# topPercent <- 0.10
# cmp the top topPercent TADs and topPercent gen# ! IN THIS VERSION, SELECTION OF TAD GENES AND GENES BASED ON PVAL
pvalSelect <- 0.05

outFold <- "CMP_TADs_GENES_GO_SIGNIF_ENTROPY_pvalSelect"
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "cmp_tads_genes_go_signif_entropy_pvalSelect_logFile.txt")
...
if(buildTable){
  ...
  if(nSelectGenes > 0){
    ...
    if(nSelectTADs_genes > 0) {
      pvals0.01_selectTADs_genes <- -log10(selectTADs_genes_resultDT$p.adjust[selectTADs_genes_resultDT$p.adjust <= 0.01])
      pvalsEntropy0.01_selectTADs_genes <- mean(pvals0.01_selectTADs_genes/sum(pvals0.01_selectTADs_genes), na.rm=T)
      
      pvals0.05_selectTADs_genes <- -log10(selectTADs_genes_resultDT$p.adjust[selectTADs_genes_resultDT$p.adjust <= 0.05])
      pvalsEntropy0.05_selectTADs_genes <- mean(pvals0.05_selectTADs_genes/sum(pvals0.05_selectTADs_genes), na.rm=T)
      
    } else {
      pvals0.01_selectTADs_genes <- NA
      pvalsEntropy0.01_selectTADs_genes <- NA
      
      pvals0.05_selectTADs_genes <-  NA
      pvalsEntropy0.05_selectTADs_genes <- NA
      
    }
    
    
    if(nSelectGenes > 0) {
      pvals0.01_selectGenes <- -log10(selectGenes_resultDT$p.adjust[selectGenes_resultDT$p.adjust <= 0.01])
      pvalsEntropy0.01_selectGenes <- mean(pvals0.01_selectGenes/sum(pvals0.01_selectGenes), na.rm=T)
      
      pvals0.05_selectGenes <- -log10(selectGenes_resultDT$p.adjust[selectGenes_resultDT$p.adjust <= 0.05])
      pvalsEntropy0.05_selectGenes <- mean(pvals0.05_selectGenes/sum(pvals0.05_selectGenes), na.rm=T)
      
    } else {
      pvals0.01_selectGenes <- NA
      pvalsEntropy0.01_selectGenes <- NA
      
      pvals0.05_selectGenes <- NA
      pvalsEntropy0.05_selectGenes <- NA
    ...
    data.frame(
      ...
      pvalsEntropy0.01_selectTADs_genes = pvalsEntropy0.01_selectTADs_genes ,
      pvalsEntropy0.05_selectTADs_genes = pvalsEntropy0.05_selectTADs_genes,
      pvalsEntropy0.01_selectGenes =  pvalsEntropy0.01_selectGenes,
      pvalsEntropy0.05_selectGenes = pvalsEntropy0.05_selectGenes,
...
all_curr_var <- c("ratioTADgenes0.05", "ratioTADgenes0.01", 
                  ...
                  "pvalsEntropy0.01_selectTADs_genes",
                  "pvalsEntropy0.05_selectTADs_genes",
                  "pvalsEntropy0.01_selectGenes",
                  "pvalsEntropy0.05_selectGenes"
                  )
...
curr_var <- c("ratioSelectGenes0.05")
for(ref_var in all_ref_var){
  for(curr_var in all_curr_var) {
    outFile <- file.path(outFold, paste0(ref_var, "_vs_", curr_var, "_signifGO.", plotType))
    do.call(plotType, list(outFile, height = myHeight, width = myWidth))
...
curr_var <- c("ratioSelectGenes0.01")
for(ref_var in all_ref_var){
  for(curr_var in all_curr_var) {
    outFile <- file.path(outFold, paste0(ref_var, "_vs_", curr_var, "_signifGO.", plotType))
    do.call(plotType, list(outFile, height = myHeight, width = myWidth))
...
for(pval in all_pval){
  for(curr_type in all_types){
    ...
    var1 <- paste0("pvalsEntropy", pval, "_", "selectTADs_genes")
    ...
    plotTit <- paste0("Pval entropy signif GOs ", pval)
    mySub <- paste0("top Genes = ", curr_type)
    p_var <- ggplot(plotDT_m, aes(x = dataset, y = value, fill = variable)) +
      ggtitle(plotTit, subtitle = mySub)+
      geom_bar(stat="identity", position = "dodge")+
      ...
      scale_fill_manual(values =setNames( c("dodgerblue4", "darkorange2"), c(var1, var2)),
                        labels =setNames( c("selectTADs_genes", curr_type), c(var1, var2))) +
      labs(fill = "")+
      theme( # Increase size of axis lines
        # top, right, bottom and left
        plot.margin = unit(c(1, 1, 2, 1), "lines"),
    ...
    if(SSHFS) p_var
    outFile <- file.path(outFold, paste0("pvalsEntropy", pval, "_selectTADs_genes", "_pvalsEntropy", pval, "_", curr_type, "_orderFCC.", plotType))
    ggsave(plot = p_var, filename = outFile, height=myHeightGG, width = myWidthGG)
    cat(paste0("... written: ", outFile, "\n"))
