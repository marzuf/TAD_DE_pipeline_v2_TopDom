startTime <- Sys.time()
cat(paste0("> Rscript cmp_DE_TADs_genes_KEGGsignifEntropy.R\n"))

# Rscript cmp_DE_TADs_genes_KEGGsignifEntropy.R
...
outFold <- "CMP_TADs_GENES_KEGG_SIGNIF_ENTROPY"
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, "cmp_tads_genes_kegg_signif_entropy_logFile.txt")
...
for(pval in all_pval){
  for(curr_type in all_types){
    ...
    if(SSHFS) p_var
    outFile <- file.path(outFold, paste0("signifKEGG_pvalsEntropyTADgenes", pval, "_pvalsEntropy", curr_type, pval, "_orderFCC.", plotType))
    ggsave(plot = p_var, filename = outFile, height=myHeightGG, width = myWidthGG)
    cat(paste0("... written: ", outFile, "\n"))
