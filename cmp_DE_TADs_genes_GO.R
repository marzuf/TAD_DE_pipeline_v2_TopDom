...
# NB: retrieved code info. from 
# 23topTable_funcEnrich_TopTableGenes_TADsGenes.R
# 23topTADs_funcEnrich_TopTableGenes_TADsGenes.R
...
all_ds_DT$nGO_topTADs_percentGenes <- (all_ds_DT$nbrGO_topTADs_genes + all_ds_DT$nbrGO_topGenes_manyAsTopPercent)
all_ds_DT$nGO_topTADs_sameNbrGenes <- (all_ds_DT$nbrGO_topTADs_genes + all_ds_DT$nbrGO_topGenes_manyAsTopTADs)

all_ds_DT$ratio_nbrPercent <- all_ds_DT$intersectNbrGO_topTADs_topGenesManyAsTopPercent/all_ds_DT$nGO_topTADs_percentGenes
all_ds_DT$ratio_sameNbr <- all_ds_DT$intersectNbrGO_topTADs_topGenesManyAsTopTADs/all_ds_DT$nGO_topTADs_sameNbrGenes
...
myx <- all_ds_DT$intersectNbrGO_topTADs_topGenesManyAsTopTADs
myy <- all_ds_DT$intersectNbrGO_topTADs_topGenesManyAsTopPercent
...
outFile <- file.path(outFold, paste0("GO_at_intersect_", "bothRatios_orderFCC.", plotType))
ggsave(plot = p_var, filename = outFile, height=myHeightGG, width = myWidthGG)
cat(paste0("... written: ", outFile, "\n"))
...
for(curr_plot in toplot) {
  if(curr_plot=="percent") {
    plotDT <- all_ds_DT[,c("dataset", "nGO_topTADs_percentGenes", "intersectNbrGO_topTADs_topGenesManyAsTopPercent")]
    plotTit <- paste0("# GO and intersect - top ", round(topPercent*100, 2), "% DE genes")
  } else if(curr_plot=="sameNbr") {
    plotDT <- all_ds_DT[,c("dataset", "nGO_topTADs_sameNbrGenes", "intersectNbrGO_topTADs_topGenesManyAsTopTADs")]
    plotTit <- paste0("# GO and intersect - as many genes as in top TADs")
  }
  plotDT_m <- melt(plotDT, id="dataset")
...
dataset_order_FCC <- as.character(all_ds_DT$dataset[order(all_ds_DT$aucFCC, decreasing=T)])
  ...
  outFile <- file.path(outFold, paste0("GO_at_intersect_", curr_plot, "TopGenes_orderFCC.", plotType))
  ggsave(plot = p_var, filename = outFile, height=myHeightGG, width = myWidthGG)
  cat(paste0("... written: ", outFile, "\n"))
